...
async function loadGraph() {
  loading.value = true
  try {
    const res = await apiFetch<any>(`/admin/courses/${courseId}/knowledge-graph`)
    if (res.data?.graph) {
...
async function generateGraph() {
  ...
  try {
    const res = await apiFetch<any>(`/admin/courses/${courseId}/knowledge-graph/generate`, { method: 'POST' })
    if (res.data?.graph) {
      graphData.value = res.data.graph as GraphData
      textbookTitle.value = res.data.textbookTitle || ''
      hasGraph.value = true
...
async function saveGraph() {
  ...
  try {
    ...
    await apiFetch(`/admin/courses/${courseId}/knowledge-graph`, {
      method: 'PUT',
      body: stored,
    })
      ...
      node: {
        ...
        style: {
          ...
          stroke: (d: any) => {
            ...
            // darken the stroke slightly for sphere look
            return fill
          },
          strokeWidth: 3,
          labelText: (d: any) => {
  ...
  <div class="kg-page">
    ...
    <div class="kg-header">
      <div class="kg-header-left">
        <el-button text :icon="ArrowLeft" @click="router.push('/admin/courses')">返回</el-button>
        <span class="kg-title">{{ courseName || '课程' }} &rsaquo; 知识图谱</span>
        <span v-if="textbookTitle" class="kg-subtitle">教材：{{ textbookTitle }}</span>
      </div>
    </div>
