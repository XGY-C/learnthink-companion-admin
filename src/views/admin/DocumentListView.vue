...
async function handleBookInfo(doc: KnowledgeDocument) {
  ...
  bookInfo.value = null
  try {
    const res = await apiFetch<BookInfo>(`/admin/documents/${doc.id}/book-info`)
...
async function handleExtractBookInfo() {
  ...
  try {
    const res = await apiFetch<BookInfo>(`/admin/documents/${doc.id}/extract-book-info`, { method: 'POST' })
...
async function loadData() {
  ...
  try {
    const [docRes, courseRes] = await Promise.all([
      apiFetch<KnowledgeDocument[]>('/admin/documents'),
...
async function handleUpload() {
  ...
  try {
    ...
    await apiFetch('/admin/documents/upload-pdf', {
      method: 'POST',
      body: formData,
    })
...
async function handleUploadMd() {
  ...
  try {
    ...
    await apiFetch('/admin/documents/upload-md', {
      method: 'POST',
      body: formData,
    })
...
async function handleParse(doc: KnowledgeDocument) {
  ...
  showParse.value = true
  try {
    const res = await apiFetch<{ taskId: string }>('/admin/documents/parse-submit', {
      method: 'POST',
...
function startParsePolling() {
  parsePollTimer.value = setInterval(async () => {
    try {
      ...
      }>(`/admin/documents/parse-task/${parseTaskId.value}`)
...
async function handleSplitMd(doc: KnowledgeDocument) {
  ...
  try {
    const res = await apiFetch<{ chapters: ChapterInfo[]; count: number }>('/admin/documents/split-md', {
      method: 'POST',
...
async function handlePreview(docId: string) {
  ...
  try {
    const res = await apiFetch<{ content: string; title: string }>(`/admin/documents/preview?id=${docId}`)
...
async function previewFullMd() {
  ...
  previewContent.value = ''
  try {
    const res = await apiFetch<{ content: string; title: string }>(`/admin/documents/preview-by-url?url=${encodeURIComponent(parseProgress.value.fullMdUrl)}`)
    previewContent.value = res.data.content
  } catch (e: any) {
...
async function submitIngest() {
  ...
  ingestProgress.value = null
  try {
    const res = await apiFetch<{ task_id: string }>('/admin/documents/ingest', {
      method: 'POST',
...
function startPolling() {
  ingestPollTimer.value = setInterval(async () => {
    try {
      const res = await apiFetch<IngestTaskProgress>(`/admin/documents/ingest-task/${ingestTaskId.value}`)
...
async function loadRecords() {
  ...
  try {
    let url = '/admin/documents/ingest-tasks?limit=20'
...
function startDetailPolling(taskId: string) {
  detailPollTimer.value = setInterval(async () => {
    try {
      const res = await apiFetch<any>(`/admin/documents/ingest-task/${taskId}`)
