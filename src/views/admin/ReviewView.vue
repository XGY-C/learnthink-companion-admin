...
async function loadQueue() {
  ...
  try {
    const [itemsRes, coursesRes, countsRes] = await Promise.all([
      apiFetch<ResourceItem[]>('/admin/review/items', { method: 'GET' }),
      apiFetch<Course[]>('/admin/courses'),
      apiFetch<{ pending: number; medium: number }>('/admin/review/counts'),
    ])
...
async function handleReview(result: 'approved' | 'rejected') {
  ...
  try {
    await apiFetch(`/admin/review/${selectedItem.value.id}`, {
      method: 'POST',
...
async function handleRetry() {
  if (!selectedItem.value) return
  try {
    await apiFetch(`/admin/review/${selectedItem.value.id}/retry`, { method: 'POST' })
